...
### 1. 優先採用 AI 驗證 (Mandatory AI Verification)
- **規範**: 初步的功能開發完成後,工程師必須優先要求 AI 啟動 Browser 工具進行自動化測試與截圖。
- **稽核價值**: 透過 AI 執行的測試會生成 `.webp` 錄影與具備時間戳記的截圖,這是證明開發者「在席且在執行驗證」的最強力物理證據。

### 2. 人工驗證紀錄化
- 若涉及複雜互動需手動測試,測試後必須將關鍵結果截圖發回給 AI。
- 嚴禁長時間「離線測試」而不與 AI 互動,否則該段時間將被系統視為 Thinking Gap (非活動時效)。
