...
## 7. 稽核導向的測試規範 (Audit-Oriented Testing Policy)

為了確保開發工時的精確性與工作連續性的可追蹤性，專案要求執行以下測試規範：

### 7.1 強制性 AI 代理測試 (Mandatory AI Subagent Testing)
- **原則**：所有的初步網頁功能測試（Initial Web Testing）應優先透過 AI 的瀏覽器執行工具（Browser Subagent）進行。
- **目的**：
    1. **自動存檔**：測試過程中的截圖、錄影及 Console Logs 會自動存入 `brain` 目錄。
    2. **工時連貫**：生成的測試檔案（.webp/.png）提供具備物理意義的時間戳記，能有效將「編碼」與「驗證」階段連鎖記錄，消除不必要的 Thinking Gap。
    3. **可稽核全貌**：讓管理者能透過稽核報告還原開發者「在測試什麼」與「測試了多久」。

### 7.2 測試行為紀錄化
- 若因環境限制必須進行手動測試，開發者應在對話中上傳測試結果截圖（如報錯畫面、成功後的 UI 狀態），作為「驗證階段」工時採信的核心證據。
